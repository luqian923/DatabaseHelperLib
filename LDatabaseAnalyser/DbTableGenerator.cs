...
public class DbTableGenerator : IIncrementalGenerator
{
    ...
    private void Execute(SourceProductionContext context, System.Collections.Immutable.ImmutableArray<(string Name, bool IsCritical, uint AllowDbId)?> entities)
    {
        ...
        aotSb.AppendLine("// <auto-generated />");
        aotSb.AppendLine("using System.Diagnostics.CodeAnalysis;");
        aotSb.AppendLine("namespace LQ.DatabaseHelper.Generated");
        aotSb.AppendLine("{");
        aotSb.AppendLine("    public static class AotEntityPreserver");
        aotSb.AppendLine("    {");
        aotSb.AppendLine("        [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(global::LQ.DatabaseHelper.LDbBaseTable))]");
        foreach (var entity in entities)
        {
            if (entity == null) continue;

            aotSb.AppendLine($"        [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof({entity.Value.Name}))]");
        }
        aotSb.AppendLine("        public static void PreserveAll() { }");
        aotSb.AppendLine("    }");
        aotSb.AppendLine("}");

        context.AddSource("AotEntityPreserver.g.cs", SourceText.From(aotSb.ToString(), Encoding.UTF8));
    }
}
